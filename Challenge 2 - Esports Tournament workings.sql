...
SELECT team_name, COUNT(player_name) AS no_of_players
FROM teams
	INNER JOIN players
   	ON teams.team_id = players.team_id
GROUP BY team_name;
...
SELECT 	team_name, COUNT(winner_id) AS no_of_matches_won
FROM teams 
	INNER JOIN matches 
...
SELECT AVG(salary)AS avg_salary, country, team_name
FROM teams
	INNER JOIN players
    	ON teams.team_id = players.team_id
WHERE country = "USA"
GROUP BY country, team_name;
